...
class AverageMeter(object):
    ...
    def update(self, val, n=1):
        self.val = val                    # 本轮的平均值
        self.sum += val * n               # 本轮的总值
        self.count += n                   # 总的计数次数
        self.avg = self.sum / self.count  # 总的均值
...
def train_validate(
    ...
    for idx, data in enumerate(train_loader):
        ...
        avg_lm_loss.update(_lm_loss.item())                   # 计算梯度，更新参数，更新计划器
        optimizer_step(
            _lm_loss/(args.grad_acc), optimizer, model, scheduler, args, is_update=is_update
        )
        
        if train_step % args.log_interval == 0:               # 打印日志记录
        ...
        if train_step % args.save_interval == 0:              # 保存模型
            if args.rank == 0:
        ...
        # evaluation interval
        if train_step % args.eval_interval == 0:              # 模型测试
...
if __name__ == '__main__':
    ...
    train_data = FT_Dataset(
        args.train_data, args.train_batch_size, args.seq_len,    # 定义数据集
    ...
    train_loader = DataLoader(                                   # 定义分布式dataloader
    ...
    lm_net = GPT2LMModel(config)                                 # 定义模型
    if args.init_checkpoint is not None:                         # 上载权重参数
    ...
    if args.lora_dim > 0:
        lora.mark_only_lora_as_trainable(lm_net)                 # 仅仅训练LoRA中的A和B
    optimizer = create_adam_optimizer_from_args(lm_net, args)    # 创建优化器，就是调用AdamW()函数

    if args.max_step is None:
        args.max_step = (args.max_epoch * train_data.num_batches + args.world_size - 1) // args.world_size
        print('set max_step:', args.max_step)

    scheduler = create_optimizer_scheduler(optimizer, args)      # 创建计划器，就是调用CosineAnnealingWarmupRestarts(), get_linear_schedule_with_warmup()等函数
    if args.fp16:
        lm_net, optimizer = amp.initialize(lm_net, optimizer, opt_level="O1")             # 适配半精度训练
    lm_net, optimizer = distributed_opt(args, lm_net, optimizer, grad_acc=args.grad_acc)  # 适配分布式训练

    try:
        train_step = 0
        for epoch in itertools.count(start=1):                    # 开始分布式训练
